...
import joblib  # Use joblib instead of pickle for model serialization/deserialization
...
select = option_menu(
    None,
    ["Home", "Readmission"],
    icons=["hospital-fill", "ticket-detailed"],
    ...
    styles={
        ...
        "nav-link-selected": {
            ...
            "color": "#000000",  # Set text color to black when selected
        },
    }
)
...
''')
    ...
    with col3:
        ...
        A1C = st.selectbox('Select the A1C Result:', ['Normal', 'Abnormal'])
        if A1C == "Normal":
            A1C_Result = 1
        else:
            A1C_Result = 0
    ...
    with col2:
        ...
        if button:
            ...
            if admission is None:
                st.write("## :red[Model could not be loaded. Please check the log for errors.]")
            elif admission == 1:
                st.write("## :red[Readmission is Required]")
            else:
                st.write("## :green[Readmission is Not Required]")

# __________________________END_____________________________________________________________
